...
class ReadExcel:
    ...
    def read_data_all(cls, file_name, sheet_name):  # 读取,文件名，sheet_name中的所有内容
        ...
        rows = list(sh.rows)  # 获取所有的cell单元格，转换成列表
        all_cases = []
        # 读取表头数据
        titles = []
    ...
    def read_data_pl(cls, file_name, sheet_name, begin_row, end_row):
        ...
        for row in rows[begin_row:end_row + 1]:  # row 代表除了表头的每一行数据
            ...
            for v in row:
                data.append(v.value)
            case_zip = dict(zip(titles, data))  # 将数据打包 {‘case_id’:1,'case_titles':'登录接口主流程'
            cd = CaseData(case_zip)  # 调用反射类，将字典数据转换为对象,放在一个迭代器里面,是字典类型
            all_cases.append(cd)
        return all_cases
...
if __name__ == "__main__":
    ...
    print(cases)
    for case in cases:
        print(case)  # 是一个迭代器，类型是字典

        print(case.case_id, case.case_title, case.case_data)

    # 读指定行的数据
    cases = ReadExcel.read_data_pl(r'./cases.xlsx', "Login", 3, 4)
    for case in cases:
        print(case.case_id, case.case_title, case.case_data)

    # 写入数据
    ReadExcel.write_data(r'cases.xlsx', 'Login', 1, 7, '成功')

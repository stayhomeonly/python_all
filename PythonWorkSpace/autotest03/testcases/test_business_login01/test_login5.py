...
class TestLogin(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.db = DBUtils()
        one = cls.db.find_one("select * from tb_user ORDER BY rand() limit 1;")
        cls.name, cls.pwd = one[1], one[2]  # 随机从数据捞取数据

    @classmethod
    def tearDownClass(cls):
        cls.db.close()

    # 读取excel中的Business_Login的数据
    cases = ReadExcel.read_data_all(DATA_FILE, 'Business_Login')

    @data(*cases)  # 参数化 data下面的cases
    ...
    def test_login(self, case):
        ...
        if case.case_id == 1:
            ...
            self.db.cud("delete from tb_user where name = %s or email= %s or phone = %s;",
                        (uname, 'test@163.com', '18655664499'))
